function valNome() {
    var nome = document.getElementById('input_nomeEmpresa').value
    ...
    if (ultimaLetra == ultimaLetraMaiuscula && ultimaLetra == ultimaLetraMinuscula || (tamanho > 254)) { // Válido apenas a entrada de letras e o limite é 255
        input_nomeEmpresa.value = input_nomeEmpresa.value.slice(0, -1)
    }
}
...
function valRepetirSenha() {
    var repetirSenha = document.getElementById('input_repetirSenhaEmpresa').value
    var tamanho = repetirSenha.length

    if (tamanho > 50) { // Não deixa colocar mais que 50 caracteres
        input_repetirSenhaEmpresa.value = input_repetirSenhaEmpresa.value.slice(0, -1)
    }
}
